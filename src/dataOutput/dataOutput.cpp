...
AclLiteError DataOutputThread::DataProcess()
{
    ...
    if (flag == postNum_) {
        for (int i = 0; i < postNum_; i++) {
            ...
            ProcessOutput(detectDataMsg);
            postQueue_[i].pop();
            //ACLLITE_LOG_INFO("THE CLASS NUM IS 111111"); //show calssnum on terminal
        }
    }
    return ACLLITE_OK;
}
...
AclLiteError DataOutputThread::SaveResultPic(shared_ptr<DetectDataMsg> &detectDataMsg)
{
    ...
    for(int i = 0; i < detectDataMsg->frame.size(); i++) {
        ...
        sstream << "../out/channel_" << detectDataMsg->channelId
            << "_out_pic_" << detectDataMsg->msgNum << i << ".jpg"; //save picture in out
        cv::imwrite(sstream.str(), detectDataMsg->frame[i]);
		//here!!! load picture
        stringstream imgname;
        imgname.str("");
        imgname << "channel_" << detectDataMsg->channelId
            << "_out_pic_" << detectDataMsg->msgNum << i << ".jpg"; //save picture in out
		//cv::Mat img = cv::imread(imgname.str(), 0);
		//system(("eog"+imgname.str()+"&").c_str());
		//system(("eog--reload" + imgname.str()).c_str());
        //here!!! save txt in out
        //stringstream fp;
        //fp.str("");
        //fp << "..out/channel_"<< detectDataMsg->channelId
        //    << "_out_pic_" << detectDataMsg->msgNum << i << ".txt"
        //FILE * fptxt;
        //fptxt = fopen(fp,"w+");
        //fprint(fptxt,"%s",fp);
        //fclose(fptxt);
